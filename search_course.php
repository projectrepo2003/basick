...
<section class="courses">
   ...
   <div class="box-container">
      ...
      <?php
         ...
         $search_course1 = "%" . $_POST['search_course'] . "%";
         $status = "active";
         //$select_courses = $conn->prepare("SELECT * FROM `playlist` WHERE title LIKE '%{$search_course}%' AND status = ?");
         $select_courses = $conn->prepare("SELECT * FROM `playlist` WHERE (title LIKE ? OR category LIKE ?) AND status = ?");
         //$select_courses->execute(['active']);
         $select_courses->execute([$search_course1, $search_course1, $status]);
